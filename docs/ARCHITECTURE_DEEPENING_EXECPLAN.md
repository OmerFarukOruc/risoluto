...
- [x] (2026-05-22 14:39Z) Removed the remaining `frontend/src/state/polling.ts` and `frontend/src/state/event-source.ts` compatibility facades after current callers and tests moved directly onto `RuntimeClient`. `frontend/src/ui/shell.ts` now dismisses stale state through `getRuntimeClient().dismissStaleBanner()`, `frontend/src/components/live-log.ts` imports the runtime event payload type from `runtime-client`, and facade behavior coverage now lives in `tests/frontend/runtime-client.test.ts`.
...
Milestone 4 is the frontend runtime client, and it is now effectively complete for the major page-level consumers. `frontend/src/state/runtime-client.ts` owns polling cadence, stale tracking, SSE connection management, reconnect logic, lifecycle-triggered refreshes, stale-banner dismissal, and browser-event dispatch over the shared store. `frontend/src/main.ts` starts that client directly, and the old `frontend/src/state/polling.ts` / `frontend/src/state/event-source.ts` migration facades have been removed.
