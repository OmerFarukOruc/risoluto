...
It polls Linear, claims eligible issues, spins up isolated Docker sandboxes, runs an AI agent inside each one, and delivers the result as a GitHub pull request.

<Frame>
```mermaid
%%{init: {'flowchart': {'defaultRenderer': 'elk'}, 'theme': 'base', 'themeVariables': {'primaryColor': '#9a472b', 'primaryTextColor': '#fff', 'primaryBorderColor': '#c96e4a', 'lineColor': '#c96e4a', 'secondaryColor': '#0c1016', 'tertiaryColor': '#1a1f2e'}}}%%
flowchart LR
    A["Linear\nIssue Tracker"] -->|"poll every 15 s"| B["Risoluto\nOrchestrator"]
    B -->|create| C["Workspace\nPer-Issue Dir"]
    C -->|mount into| D["Docker\nSandbox"]
    D -->|execute| E["Codex\nAI Agent"]
...
<Steps>
  <Step title="Polling" icon="satellite-dish">
    Risoluto polls Linear every **15 seconds** (configurable via `polling.interval_ms`).
