...
| `research/INDEX.md` *(private submodule)* | Flat ledger of researched targets (slug, type, source, revision, captured, features, artifact, status). **Spine-free** — no Risoluto comparison. Per-target detail under `research/targets/<slug>.md`. Older spine-aligned ledgers live frozen under `research/legacy/`. Absent when submodule isn't initialized — degrade gracefully. |
| `research/RISOLUTO_FEATURES.md` *(private submodule)* | Canonical feature spine, owned by `/update-feature-spine`. Read by the future cross-target harmonization skill that joins it with `research/targets/*.md` to compute feature-parity claims. The `/risoluto-researcher` skill no longer reads or writes this file. |
