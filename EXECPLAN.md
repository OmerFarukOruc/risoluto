...
- [x] (2026-03-16 02:29Z) Switched the checked-in local `WORKFLOW.md` to the repo-local minimal Codex home fixture and revalidated that the live smoke path starts without the earlier broken-skill warnings from `~/.codex`.
- [x] (2026-03-16 02:39Z) Fixed CLIProxyAPI authentication for the live Symphony workflow by seeding the isolated Codex runtime from the fixture home while explicitly exporting the working bearer key from `~/.codex/auth.json`.
...
- [x] (2026-03-16 12:40Z) Persisted `.symphony/issue-index.json` alongside attempt archives, added the repo-root `./symphony-logs` inspection helper plus a controlled archive sandbox fixture, and refreshed the release docs for `v0.1.1`.
...
- Decision: use the checked-in minimal Codex home fixture for the repository's live `WORKFLOW.md`, while keeping `WORKFLOW.example.md` generic and isolated under `~/.symphony-codex`.
  Rationale: this removes noisy failures inherited from the operator's personal `~/.codex` without forcing machine-global changes, and keeps the example workflow portable.
  Date/Author: 2026-03-16 / Codex

- Decision: keep the live launcher isolated, but explicitly export `OPENAI_API_KEY` from `~/.codex/auth.json` when starting Codex against CLIProxyAPI.
  Rationale: local evidence showed the proxy accepts the bearer key from `auth.json`, while the placeholder `api-keys` entry in `~/.cli-proxy-api/config.yaml` is not the working client credential for this machine.
  Date/Author: 2026-03-16 / Codex
...
The current repo also includes a parity-focused runtime hardening pass: hook timeout support, codex turn timeout support, corrected thread/token usage aggregation, running-state issue detail fixes, a live dashboard that matches the provided stitch mock much more closely while staying backed by the real API surface, a corrected shutdown path that no longer queues retries after `SIGINT` or other non-retriable worker stops, a fixed live launcher that authenticates properly to the local CLIProxyAPI while still avoiding the broken-skill noise from the operator's main `~/.codex` home, issue-level model routing controls that save the next-run model and reasoning pair from the website or API without interrupting an active worker, and a durable attempt archive under `.symphony/` with archived-run API access and dashboard tabs. The archive now also persists `issue-index.json` for fast issue-to-attempt lookups, and the repo-root `./symphony-logs` helper plus the controlled sandbox fixture make historical run inspection reproducible without touching live production data. A fresh live smoke run with the checked-in `WORKFLOW.md` now starts on `gpt-5.4/high`, drives real `200` responses through the proxy, saves a pending next-run override to `gpt-5/medium` through the override endpoint, and writes the attempt plus its event stream to disk for later inspection.
