...
curl -sf -X POST "${BASE}/templates" -H "Content-Type: application/json" -d '{"id":"default-agent","name":"Default Agent Prompt","body":"You are an autonomous agent working on {{ issue.identifier }}: {{ issue.title }}.\n\n{{ issue.description | default: \"No description.\" }}\n\nWork in {{ workspace.path }}. Fix the issue, write tests, submit a PR."}' > /dev/null 2>&1 && ok "Template: default-agent" || warn "default-agent exists"
curl -sf -X POST "${BASE}/templates" -H "Content-Type: application/json" -d '{"id":"careful-review","name":"Careful Review Prompt","body":"Review {{ issue.identifier }}: {{ issue.title }}.\nPriority: {{ issue.priority | default: \"normal\" }}\n\nIdentify root cause, minimal fix, verify with tests."}' > /dev/null 2>&1 && ok "Template: careful-review" || warn "careful-review exists"
