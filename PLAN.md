...
| ------------------------------------------------------ | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Align stale current-state architecture module names    | Strong      | Fresh scan after `aa225a4` showed `docs/ARCHITECTURE_DEEPENING_EXECPLAN.md` still gives current guidance for deleted or renamed modules: `src/agent-runner/codex-runtime-port.ts`, `src/setup/api.ts`, `frontend/src/features/settings/unified-settings-page.ts`, `frontend/src/features/settings/settings-workbench.ts`, and `frontend/src/views/unified-settings-view.ts`. Current source uses `session-port.ts`, `setup-service.ts`/`port.ts`, and `settings-page.ts`/`workbench.ts`. | Must preserve historical entries while correcting current guidance and summaries.   | The goal treats docs/tests/runtime disagreement as architecture friction. Stale module/interface names reduce future locality and leverage.      |
| Keep webhook composition compatibility surface         | Speculative | `src/webhook/composition.ts` is marked as compatibility, but current CLI/service wiring uses it to compose persistence, health, registration, secrets, and handler deps behind a stable service-level interface.                                                                                                                                                                                                                        | Deleting it would push runtime assembly into CLI/service wiring and reduce locality. | Do not implement unless repeated webhook runtime ownership appears in current callers.                                                           |
| Keep setup default-branch route adapter as-is          | Speculative | `src/setup/detect-default-branch.ts` still owns the route adapter for `/api/v1/setup/detect-default-branch` while delegating behavior to `SetupService`.                                                                                                                                                                                                                                                                                              | Removing or widening it would risk route contract churn with weak leverage.         | Do not implement unless route adapter behavior becomes duplicated elsewhere.                                                                      |
| Keep remaining frontend shell listener cleanup as-is   | Speculative | Architecture docs explicitly classify remaining raw browser listeners as shared shell compatibility seams; current source does not show repeated page-level ownership after the runtime-client facade removal.                                                                                                                                                                                                                                           | UI behavior validation cost is high and leverage evidence is weak.                  | Do not implement without a concrete repeated-ownership signal in a feature module.                                                               |

### Chosen Candidate

- Candidate: Align stale current-state architecture module names
- Reason chosen: Strong. Current source has deeper modules/interfaces in place, but the architecture execution plan still describes deleted or renamed files as current guidance. That docs/runtime disagreement would mislead the next architecture iteration.
- Current problem: `docs/ARCHITECTURE_DEEPENING_EXECPLAN.md` preserves useful history, but its current-state plan, outcomes, and interface sections still point future contributors to non-existent or stale module names.
- Deeper module/interface being created or simplified: No runtime module is changed. This slice updates the architecture documentation interface so future agents see the real implementation state: `AgentSessionPort`/`AgentSession` in `src/agent-runner/session-port.ts`, setup onboarding behind `SetupPort`/`SetupService`, and settings behind `createSettingsPage()`/`SettingsWorkbench`.
- Affected files:
  - `docs/ARCHITECTURE_DEEPENING_EXECPLAN.md`
  - `PLAN.md`, `ATTEMPTS.md`, `NOTES.md`
- Compatibility expectations: Documentation-only update. Preserve historical entries and only correct current-state guidance, summaries, and stable interface references.
- Tests to add or update: None; no runtime behavior changes.
- Docs or ADRs to update: Update stale current-state guidance in the architecture execution plan and add a revision note for the module-name alignment.
- Validation commands:
