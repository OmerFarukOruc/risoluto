...
- [x] (2026-03-22) Implemented resilience foundation (branch `feature/resilience-foundation`, v0.3.1): added `src/orchestrator/stall-detector.ts` (orchestrator-level stall kill + `StallEvent` records), `src/orchestrator/watchdog.ts` (60 s background health monitor with `healthy`/`degraded`/`critical` states), `src/linear/client.ts` write-back methods (`resolveStateId`, `updateIssueState`, `createComment`), `src/orchestrator/worker-outcome.ts` Linear completion write-back (non-blocking, 3-retry), and extended `AgentConfig` with `successState` and `stallTimeoutMs`. Wired stall detector and watchdog into orchestrator lifecycle. Added `src/linear/transition-query.ts` comment mutation builder and refactored state lookup to reuse `resolveStateId` in `transition-handler.ts`. New tests: `stall-detector.test.ts`, `watchdog.test.ts`, `linear-writeback.test.ts`, `write-linear-completion.test.ts` (221 + 194 + 146 + 210 lines).
