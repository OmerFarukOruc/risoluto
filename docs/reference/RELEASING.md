...
Run the minimum validation commands and confirm all pass:
...
Run the CI-mirror validation before a public release:
...
If real credentials are available, also run:
...
| Scope and limitations | Known current scope |
...
- Package-manager distribution if the release is still source-only.
