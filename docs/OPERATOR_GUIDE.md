...
- webhooks are the primary trigger path
- polling remains enabled as anti-entropy and outage recovery
- when webhook health is connected, polling stretches to a slower interval
- when webhook health degrades or disconnects, polling falls back to the base interval

This is safer than webhook-only mode because it keeps a reconciliation path for tunnel failures, disabled subscriptions, and operator mistakes.

### Architecture

```
Linear ──POST──▶ webhooks.risolu.to ──▶ Cloudflare Tunnel ──▶ risoluto:4000/webhooks/linear
```
...
Use Cloudflare Tunnel to expose your local Risoluto instance:
...
**2. Store the required credentials**

- Put **infrastructure secrets** such as `CLOUDFLARE_TUNNEL_TOKEN` in `.env`
- Store **application credentials** such as `LINEAR_API_KEY`, `OPENAI_API_KEY`, and `GITHUB_TOKEN` in the setup wizard or the encrypted secrets store
- Store the Linear signing secret in the encrypted secrets store and reference it from the config overlay
...
> 4. use the Overview page's webhook health panel to confirm delivery status

**4. Verify it works**

```bash
# Check Risoluto logs for webhook health
docker compose logs -f risoluto | grep webhook

# Should see: "webhook health: disconnected → connected"

# Unsigned POST should return 401 signature_missing through the public URL
curl -i -X POST https://webhooks.risolu.to/webhooks/linear \
  -H 'Content-Type: application/json' \
  -d '{}'
```
...
Linear webhook CRUD requires admin rights. If your Linear key is not admin-scoped, use manual mode and keep the signing secret in the encrypted secrets store instead of plaintext overlay.
