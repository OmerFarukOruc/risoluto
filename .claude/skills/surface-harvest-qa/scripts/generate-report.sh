...
# Parse metrics from coverage summary
...
# Handle both "| PASS | 131 |" and "| **PASS** | 131 |" formats
pass_count = extract(r'\|\s*\*{0,2}PASS\*{0,2}\s*\|\s*(\d+)', coverage_text, "0")
fail_count = extract(r'\|\s*\*{0,2}FAIL\*{0,2}\s*\|\s*(\d+)', coverage_text, "0")
blocked_count = extract(r'\|\s*\*{0,2}BLOCKED\*{0,2}\s*\|\s*(\d+)', coverage_text, "0")
skip_count = extract(r'\|\s*\*{0,2}SKIP\*{0,2}\s*\|\s*(\d+)', coverage_text, "0")
total_count = extract(r'\|\s*\*{0,2}Total\*{0,2}\s*\|\s*\*{0,2}(\d+)', coverage_text, "0")

# If Total not found, compute from components
total = int(total_count) or (int(pass_count) + int(fail_count) + int(blocked_count) + int(skip_count)) or 1
pass_pct = round(int(pass_count) / total * 100, 1)
...
# ── Parse per-route breakdown from manifest ───────────────────────────────────

route_sections = []
current_route = None
for line in coverage_text.split("\n"):
    if line.startswith("| ") and "Route" not in line and "---" not in line and "Status" not in line:
        cols = [c.strip() for c in line.split("|")[1:-1]]
        if len(cols) >= 5:
            route_sections.append(cols)
...
routes_html = ""
for cols in route_sections:
    name = cols[0]
    p = cols[2] if len(cols) > 2 else "0"
    f = cols[3] if len(cols) > 3 else "0"
    b = cols[4] if len(cols) > 4 else "0"
    s = cols[5] if len(cols) > 5 else "0"
    routes_html += f'''
    <tr>
      <td>{esc(name)}</td>
      <td class="col-pass">{p}</td>
      <td class="col-fail">{f}</td>
      <td class="col-blocked">{b}</td>
      <td class="col-skip">{s}</td>
    </tr>'''
...
for entry in log_entries:
    ...
    result_class = "pass" if result == "success" else "fail"
    log_html += f'''
    <tr>
...
<div class="dashboard">
  ...
  <div class="metrics">
    <div class="metric"><div class="metric-value">{total_count}</div><div class="metric-label">Total Surfaces</div></div>
...
<table>
  ...
  <tbody>{routes_html if routes_html else '<tr><td colspan="5" class="text-muted">No per-route data found in coverage-summary.md</td></tr>'}</tbody>
</table>
