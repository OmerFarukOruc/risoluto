...
Risoluto's functionality is production-ready but the dashboard UI needs a comprehensive quality pass before v1.0. The app has a copper-accent, zero-radius "stitch" design language documented in `.impeccable.md`, but the implementation has inconsistencies, rough edges, and missing polish across 15 unique pages (16 screenshottable views). The goal is a prompt that orchestrates the entire polish workflow end-to-end without exhausting the main session's context window. (see origin: `docs/brainstorms/2026-04-01-v1-ui-ux-polish-requirements.md`)
...
- R6. Verification: `pnpm run build && pnpm run lint && pnpm run format:check && pnpm test`. After CSS/view changes, also smoke tests. After UI-affecting changes, also `/visual-verify` (see origin + NEW-1 amendment)
- R7. Auto-fix with build-error-resolver subagent, escalate after 2 failed attempts (see origin)
- R8. Do not block pipeline on minor issues -- only escalate genuine build/test failures. See Verification Tiers below for precise criteria (see origin + AI-4 amendment)
...
- R11. Baseline screenshots of all 16 views in both dark and light themes (see origin + AI-1 correction)
- R12. After-screenshots of all views at the end (see origin)
- R13. Screenshots saved to `screenshots/baseline/` and `screenshots/after/` (see origin). Screenshots are qualitative review evidence for user comparison at phase gates; deterministic visual regression is handled by the Playwright visual test suite (`--project=visual`) (see NEW-2 amendment)
...
Three tiers define what constitutes a pipeline-stopping failure vs a non-blocking observation (amendment from AI-4):

| Tier | Trigger | Action |
|------|---------|--------|
| **Blocking** | Non-zero exit from `pnpm run build`, `pnpm run lint`, `pnpm test`, or smoke tests | Trigger build-error-resolver flow (D6) |
| **Auto-fixable** | `pnpm run format:check` fails | Run `pnpm run format`, re-check, commit the fix -- do not escalate |
| **Non-blocking** | Diagnosis findings, polish observations, style preferences, minor suggestions | Log in summary, do not stop the pipeline |
...
| Smoke tests | 117 tests / 16 `*.smoke.spec.ts` files (121 tests / 17 files including `setup-gate.spec.ts`) | `tests/e2e/specs/smoke/` |
...
**16 screenshottable views** (corrected from original "17" -- no standalone `/runs` route exists; runs are only accessible via `/issues/:id/runs`). The origin requirements document should be updated to reflect this correction (AI-1 amendment).

**Parameterized route strategy (OP-7 amendment):** Routes like `/issues/:id`, `/issues/:id/runs`, `/logs/:id`, and `/attempts/:id` require real data to render meaningful content. The baseline screenshot subagent must first discover valid IDs from runtime state (e.g., query the `/api/v1/issues` endpoint) or use the mock data infrastructure from the E2E test layer (`tests/e2e/mocks/scenarios/`). If no data is available, explicitly classify those screenshots as empty-state captures and note the gap in the baseline manifest.
...
### D1: Subagent Dispatch Pattern -- Agent Tool with Skill Invocation via SlashCommand

**Decision:** Each subagent is spawned via the `Agent` tool (formerly `Task`) with a prompt that instructs it to invoke a specific slash command skill using Claude Code's **SlashCommand tool**. The SlashCommand tool is a documented Claude Code feature (added in v1.0.123) that enables programmatic skill invocation. Subagents running in a forked context can invoke skills via this mechanism (confirmed in v2.1.0: "Added support for running skills and slash commands in a forked sub-agent context").

**Rationale:** The Agent tool creates a fresh context window for each subagent. The subagent receives a focused prompt describing: (1) which skill to invoke via the SlashCommand tool, (2) which files/pages to focus on, (3) what state to read from disk, (4) what state to write to disk, (5) verification commands to run. This keeps the orchestrator's context minimal -- it only sees the subagent's final summary, not all the skill's intermediate work.
...
Agent tool call:
  prompt: "You are a UI/UX subagent. Your task:
    ...
    3. Invoke the /<skill-name> skill (via the SlashCommand tool) against the running app at http://localhost:5173
...
### D3: Dev Server Lifecycle -- Orchestrator Starts Once, Subagents Reuse, with Health Monitoring

**Decision:** The orchestrator starts the dev server once in Phase 0 and keeps it running throughout. Subagents connect to the already-running server at `http://localhost:5173`. The orchestrator performs a health check before each subagent dispatch (OP-6 amendment).

**Rationale:** Starting/stopping the dev server per subagent wastes time and creates race conditions. The Vite dev server supports HMR, so subagents' file changes are reflected immediately. However, over a multi-hour pipeline run the server can crash (malformed CSS, OOM from accumulated HMR updates, syntax errors). A pre-dispatch health check catches this.

**Server startup command:** `pnpm run dev -- --port 4000` (background process)

**Pre-dispatch health check (runs before every subagent):**
```bash
curl -sf http://localhost:5173 > /dev/null || {
  echo "Dev server is down -- restarting..."
  pnpm run dev -- --port 4000 &
  sleep 3
  curl -sf http://localhost:5173 > /dev/null || {
    echo "ERROR: Dev server failed to restart. Escalating to user."
    exit 1
  }
}
```
...
**Screenshot purpose:** Screenshots are **qualitative review evidence** for the user to visually compare progress at phase gates. They are not pixel-perfect deterministic diffs. For deterministic visual regression testing, the pipeline relies on the Playwright visual test suite (`pnpm exec playwright test --project=visual`) (NEW-2 amendment).
...
**Decision:** When a skill subagent's verification fails, the orchestrator spawns a `build-error-resolver` subagent with the error output. The build-error-resolver is dispatched via the **Agent tool** using the agent definition at `~/.claude/agents/build-error-resolver.md` (it is an agent, not a skill -- do not invoke via SlashCommand). Override its default commands with repo-specific ones: `pnpm run build` (not `npm run build`), `pnpm run lint`, `pnpm test` (OP-2 amendment). If the fix subagent fails twice, the orchestrator escalates to the user via AskUserQuestion.

**Flow:**
1. Skill subagent runs verification, reports failure
2. Orchestrator spawns build-error-resolver subagent with: error output, list of files changed by the skill, instruction to fix using repo-specific `pnpm` commands
...
### D8: Subagent Crash Recovery (OP-12 amendment)

**Decision:** If a subagent returns without writing its expected `<skill-name>-summary.md` file, the orchestrator follows this explicit recovery flow:

1. **Auto-retry once:** Re-dispatch the same subagent with the same prompt
2. **If still missing:** Ask the user via AskUserQuestion: "The /<skill-name> skill did not produce a summary. (a) Re-run it, (b) Skip it and continue, (c) I will handle it manually."
3. **If skipped:** Write a stub summary noting the skip so downstream subagents have context

This prevents silent context gaps when a subagent crashes or times out without completing its state handoff.

### D9: Context Compaction (OP-5 amendment)

**Decision:** At each phase boundary (after the phase gate AskUserQuestion), the orchestrator summarizes prior phase outcomes into the `.ui-polish/state/phase-gate-decisions.md` file and references that file rather than retaining inline conversation history from prior phases.

**Rationale:** The estimated orchestrator context footprint is ~23-37K tokens (well within 200K), but compaction prevents accidental bloat from verbose user feedback or multiple error recovery cycles. This is cheap insurance, not an existential fix.
...
- **Q: Should the orchestrator start the dev server once or each subagent manage its own?** (Deferred from requirements)
  Resolution: Orchestrator starts once in Phase 0, subagents reuse. Pre-dispatch health check ensures liveness. See D3.
...
Orchestrator                          Subagent (Agent tool)
    ...
    |                                       |-- read .impeccable.md
    |                                       |-- invoke /<skill> via SlashCommand tool
    |                                       |-- commit changes
    |                                       |-- run verification
    |                                       |   |
    |                                       |   |-- [PASS] write summary
    ...
    |<-- summary or error ------------------|
    |
    |-- [if no summary file] retry once (D8)
    |-- [if error] spawn build-error-resolver (Agent tool, not Skill)
...
- [ ] **Unit 1: Prompt Header and Critical Instructions**
  ...
  **Goal:** Write the opening section of the prompt that establishes identity, constraints, the subagent architecture contract, verification tiers, and crash recovery protocol.

  **Requirements:** R1, R2, R8, R14, R15, R16, R17, R18
  ...
  **Approach:**
  ...
  - Define the subagent contract: one Agent tool call per skill, fresh context, disk-based state handoff. Skills are invoked via the **SlashCommand tool** (not natural-language prompting) -- see D1
  ...
  - Include the verification tiers (blocking / auto-fixable / non-blocking) -- see Verification Tiers section
  - Include the error recovery protocol (build-error-resolver via Agent tool, 2-attempt escalation) -- see D6
  - Include the crash recovery protocol (missing summary file: auto-retry once, then ask user) -- see D8
  - Include the `.ui-polish/state/` directory convention
  - Include `.gitignore` instruction: add `.ui-polish/`, `screenshots/baseline/`, `screenshots/after/` to `.gitignore` in Phase 0 (AI-3 amendment)
  ...
  **Test scenarios:**
  ...
  - Crash path: orchestrator knows exactly what to do when a subagent returns without writing state files

  **Verification:**
  - A reader can identify: what the orchestrator does, what subagents do, how state flows, how errors are handled, how crashes are recovered
...
- [ ] **Unit 2: Technical Context and Codebase Reference**
  ...
  **Approach:**
  ...
  - Include the parameterized route data strategy: discover valid IDs from API or mock data before capturing those views (OP-7 amendment)
  ...
  - Note: smoke test count is 117 tests across 16 `*.smoke.spec.ts` files (or 121 across all 17 spec files including `setup-gate.spec.ts`) -- reference structurally ("run all smoke tests in `tests/e2e/specs/smoke/`") rather than asserting a specific count that may drift
...
- [ ] **Unit 3: Subagent Prompt Template**
  ...
  **Approach:**
  ...
  - Template sections: (1) role and task, (2) context files to read, (3) state files to read, (4) skill invocation via SlashCommand tool, (5) commit instructions, (6) verification (including `/visual-verify` for UI-changing phases), (7) output summary format, (8) state files to write
...
- [ ] **Unit 4: Phase 0 -- Setup and Baseline**
  ...
  **Goal:** Write the Phase 0 section covering dev server startup, `.impeccable.md` verification, `.gitignore` setup, baseline screenshots, and initial state directory creation.
  ...
  **Approach:**
  ...
  - Orchestrator adds `.ui-polish/`, `screenshots/baseline/`, `screenshots/after/` to `.gitignore` if not already present (AI-3 amendment)
  - Orchestrator starts dev server in background: `pnpm run dev -- --port 4000`
  - Orchestrator runs health check with retry: `curl -sf http://localhost:5173 > /dev/null` (D3)
  ...
  - Orchestrator spawns a "baseline-screenshot" subagent that uses `agent-browser` to capture all 16 views x 2 themes = 32 screenshots. For parameterized routes, the subagent first discovers valid IDs from the API or notes them as empty-state captures (OP-7 amendment)
  ...
  **Verification:**
  - 32 screenshots exist in `screenshots/baseline/` (some may be empty-state captures for parameterized routes)
...
- [ ] **Unit 5: Phase 1 -- Diagnosis (dogfood, critique, audit)**
  ...
  **Approach:**
  - Pre-dispatch health check: `curl -sf http://localhost:5173 > /dev/null` (D3 amendment -- runs before each subagent)
  ...
  - After each subagent returns, verify the summary file exists (D8 crash recovery)
  ...
  - Context compaction: after gate, summarize Phase 1 outcomes into state file (D9)
...
- [ ] **Unit 6: Phase 2 -- Foundation (distill, arrange, typeset)**
  ...
  **Approach:**
  - Pre-dispatch health check before each subagent (D3)
  ...
  - After each subagent, run `/visual-verify` to satisfy the repo's mandatory UI-change verification contract (NEW-1 amendment)
  - After each subagent returns, verify summary file exists (D8 crash recovery)
  - Phase 2 gate: orchestrator presents before/after screenshots of changed pages, asks "does this feel right?"
  - Context compaction: after gate, summarize Phase 2 outcomes into state file (D9)
...
- [ ] **Unit 7: Phase 3 -- Visual Identity (colorize, bolder/quieter, clarify)**
  ...
  **Approach:**
  ...
  - Each commits atomically, runs verification (including `/visual-verify` for UI-changing phases -- NEW-1 amendment)
  - After each subagent, verify summary file exists (D8 crash recovery)
  - Phase 3 gate: orchestrator presents visual identity changes, asks "how does the personality feel?"
  - Context compaction: after gate, summarize Phase 3 outcomes into state file (D9)
...
- [ ] **Unit 8: Phase 4 -- Production Hardening (harden, adapt, onboard)**
  ...
  **Approach:**
  ...
  - Each commits atomically, runs verification (including `/visual-verify` -- NEW-1 amendment)
  - After each subagent, verify summary file exists (D8 crash recovery)
  - Phase 4 gate: orchestrator shows error states, mobile views, empty states, asks "anything missing?"
  - Context compaction: after gate, summarize Phase 4 outcomes into state file. This is the midpoint of the pipeline -- a good place for thorough compaction to keep the orchestrator lean for Phases 5-7 (D9 + OP-5 amendment)
...
- [ ] **Unit 9: Phase 5 -- Delight (animate, delight)**
  ...
  **Approach:**
  ...
  - Context compaction: after gate, summarize Phase 5 outcomes into state file (D9)
...
- [ ] **Unit 10: Phase 6 -- Consolidation (normalize, extract, optimize)**
  ...
  **Approach:**
  ...
  - Context compaction: after gate, summarize Phase 6 outcomes into state file (D9)
...
- [ ] **Unit 11: Phase 7 -- Final Polish, Screenshots, and Report**
  ...
  **Approach:**
  ...
  - Runs verification including `/visual-verify` (NEW-1 amendment)
  - After-screenshot subagent: same 16 views x 2 themes = 32 screenshots to `screenshots/after/`
  - Orchestrator: present before/after comparison for each view (qualitative evidence -- see D4/R13)
...
- [ ] **Unit 12: Assembly and Prompt File Quality Pass**
  ...
  **Approach:**
  ...
  - Check that the prompt does not exceed reasonable length for a paste-in prompt (target: under 800 lines, acceptable up to ~1000 lines given token budget is ~23-37K)
  - Verify `.ui-polish/` and `screenshots/` directories are covered in `.gitignore` instruction (AI-3 amendment)
  - Verify verification tiers are clearly defined (AI-4 amendment)
  - Verify `/visual-verify` is included in post-UI-change verification steps (NEW-1 amendment)
  - Verify crash recovery protocol (D8) and context compaction rule (D9) are present
  ...
  **Verification:**
  ...
  - Prompt is under ~800-1000 lines
  - Every requirement (R1-R18) is traceable to a specific prompt section

## System-Wide Impact

- **Interaction graph:** The prompt orchestrates Agent tool calls (with SlashCommand-based skill invocation), AskUserQuestion, agent-browser, and build/test commands. Subagents interact with the Vite dev server, the filesystem (CSS/TS files), git, and the skill system.
...
- **State lifecycle risks:** If a subagent crashes without writing its summary, the orchestrator follows the D8 crash recovery protocol (auto-retry once, then ask user).
...
|------|-----------|--------|------------|----------|
| Context window exhaustion in main session | Low | High | Estimated ~23-37K tokens of 200K. Per-phase compaction (D9) prevents accidental bloat | Restart from last phase gate |
| Subagent fails to invoke skill correctly | Medium | Medium | Template explicitly names SlashCommand tool (D1); test with first skill | Manual skill invocation |
| Dev server crashes mid-pipeline | Medium | Medium | Pre-dispatch health check with auto-restart before every subagent (D3) | Restart dev server, resume from last phase |
| Cumulative CSS changes break visual consistency | Medium | Medium | `/normalize` in Phase 6 catches drift; `/polish` in Phase 7 is final pass; per-skill atomic commits enable targeted reverts (D5) | Git revert individual skill commits |
| Subagent crashes without writing state files | Low | Medium | D8 crash recovery: auto-retry once, then ask user (retry/skip/manual) | Skip skill and continue |
| Parameterized routes show empty states | Medium | Low | Discover valid IDs from API or classify as empty-state captures (OP-7) | Screenshot what's available, note gaps |
| Prompt too long to paste into session | Low | Medium | Target under ~800-1000 lines; token budget is manageable (~23-37K) | Split into two sessions (Phase 0-3, Phase 4-7) |
...
- The `screenshots/` directory provides qualitative visual before/after evidence (not deterministic diffs)
