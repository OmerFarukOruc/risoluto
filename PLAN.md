...
| ---------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Deepen notification webhook delivery implementation              | Strong      | `src/notification/slack-webhook.ts` and `src/notification/webhook-channel.ts` duplicate timeout setup, `fetch` POST mechanics, JSON content headers, non-2xx body reads, logger context, error rethrow, and timeout cleanup. They have real adapter variation in payload shape and channel filtering.             | Must preserve Slack-specific verbosity, min-severity filtering, payload format, generic webhook payload format, custom headers, error message prefixes, timeout behavior, and logger context. | Two concrete notification adapters share the same outbound delivery implementation, so this is a real seam with adapter variation rather than a hypothetical seam. |
...
| Split GitHub health probe result parsing into separate module    | Speculative | `src/health/runtime/github-http.ts` has local parsing helpers, but they are specific to the `GithubProbeHttp` adapter and not repeated elsewhere.                                                                                                                                                                  | Extraction would create a small hypothetical seam with one adapter and weak leverage.                                                                                         | Keep parsing local unless another probe or adapter repeats it.                                                                                                      |
...
- Candidate: Deepen notification webhook delivery implementation
- Reason chosen: Strong current evidence of repeated ownership with two real adapters. Slack and generic webhook channels should own event gating and payload shape, while a deeper delivery module should own the shared outbound HTTP implementation.
- Current problem: `src/notification/slack-webhook.ts` and `src/notification/webhook-channel.ts` each implement the same timeout, abort, POST, JSON header, non-2xx body read, logger context, and cleanup behavior. This weakens locality because delivery bugs or behavior tweaks must be made twice.
- Deeper module/interface being created or simplified: Add a notification webhook delivery module whose interface accepts the channel name, URL, payload, optional headers, timeout/fetch/logger dependencies, event context, and failure label. Keep Slack and generic webhook adapters as the payload/filtering seam and move only shared delivery implementation behind the new module.
- Affected files:
  - `src/notification/webhook-delivery.ts`
  - `src/notification/slack-webhook.ts`
  - `src/notification/webhook-channel.ts`
  - `tests/notification/webhook-delivery.test.ts`
  - `tests/notification/slack-webhook.test.ts`
  - `tests/notification/webhook-channel.test.ts`
  - `PLAN.md`, `ATTEMPTS.md`, `NOTES.md`
- Compatibility expectations: Slack and generic webhook notifications keep the same public behavior. Slack verbosity and min-severity filtering remain local. Generic min-severity filtering remains local. Slack payload, generic payload, custom generic headers, error messages, timeout abort, logger fields, and thrown errors stay compatible.
- Tests to add or update: Add direct unit tests for the shared delivery module and keep Slack/generic channel tests green through their public `NotificationChannel.notify()` interface.
- Docs or ADRs to update: None; operator-visible notification behavior is unchanged.
- Validation commands:
  - `pnpm exec vitest run tests/notification/webhook-delivery.test.ts tests/notification/slack-webhook.test.ts tests/notification/webhook-channel.test.ts tests/notification/manager.test.ts`
