...
| ---------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Remove stale setup handler Linear GraphQL helper seam            | Strong      | `src/setup/handlers/shared.ts` still owns raw Linear GraphQL fetch, response parsing, API-key lookup, and project lookup implementation, but current setup runtime uses `SetupService` and `LinearClient`; source callers only import `SetupApiDeps` from this file. Its tests now protect dead code. | Must keep setup route behavior unchanged and preserve handler type imports through the real setup interface.                                                   | The deeper setup module already exists. Removing the stale implementation improves locality by making setup behavior live in one implementation instead of a dead seam. |
...
- Candidate: Remove stale setup handler Linear GraphQL helper seam
- Reason chosen: Strong current evidence that the old helper seam fails the deletion test. Deleting its implementation does not force complexity into runtime callers because setup behavior already lives behind the deeper `SetupService` and `LinearClient` interfaces.
- Current problem: `src/setup/handlers/shared.ts` still presents a raw Linear GraphQL implementation and dedicated tests, but current source only needs it for the `SetupApiDeps` type. This weakens locality because readers and tests can mistake dead handler-level Linear behavior for the active setup implementation.
- Deeper module/interface being created or simplified: Simplify the setup handler interface so type-only imports point directly at `src/setup/port.ts`. Remove the stale helper implementation and its tests, leaving setup behavior behind `SetupService` and tracker/Linear adapters.
- Affected files:
  - `src/setup/handlers/shared.ts`
  - `src/setup/handlers/index.ts`
  - `src/setup/handlers/*.ts`
  - `src/setup/detect-default-branch.ts`
  - `src/setup/repo-route-handlers.ts`
  - `tests/setup/shared-handler.test.ts`
  - setup handler tests with type-only imports
  - `PLAN.md`, `ATTEMPTS.md`, `NOTES.md`
- Compatibility expectations: Setup routes keep the same runtime behavior. No public setup endpoint response changes. The only removed behavior is dead helper implementation not imported by production source.
- Tests to add or update: Remove the dead helper unit test and keep setup handler/service tests green through the active `SetupService` interface.
- Docs or ADRs to update: None; operator-visible setup behavior is unchanged.
- Validation commands:
  - `pnpm exec vitest run tests/setup/api.test.ts tests/setup/linear-project-handler.test.ts tests/setup/handlers.integration.test.ts tests/setup/master-key-handler.test.ts tests/setup/reset-handler.test.ts tests/setup/status-handler.test.ts`
